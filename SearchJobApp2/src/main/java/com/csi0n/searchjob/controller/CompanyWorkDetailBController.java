...
import com.csi0n.searchjob.utils.bean.CompanyWorkDetailBListBean;
...
public class CompanyWorkDetailBController extends BaseController implements BGARefreshLayout.BGARefreshLayoutDelegate {
    ...
    private void getHeadFromNet() {
       PostParams params = getDefaultPostParams(R.string.url_getSearchJobDetailBHeader);
        params.put("company_id", String.valueOf(mCompanyWorkDetailFragmentB.getCompanyJobBean().getId()));
        HttpPost post = new HttpPost(params, new ObjectHttpCallBack<SearchJobDetailBHeaderBean>(SearchJobDetailBHeaderBean.class) {
            @Override
            public void SuccessResult(SearchJobDetailBHeaderBean result) throws JSONException {
    ...
    private void getJob(final int page) {
        is_busy = true;
        PostParams params = getDefaultPostParams(R.string.url_getSearchJobDetailB);
        params.put("company_id", String.valueOf(mCompanyWorkDetailFragmentB.getCompanyJobBean().getId()));
        params.put("page", String.valueOf(page));
        HttpPost post = new HttpPost(params, new ObjectHttpCallBack<CompanyWorkDetailBListBean>(CompanyWorkDetailBListBean.class) {
            @Override
            public void SuccessResult(CompanyWorkDetailBListBean result) throws JSONException {
                if (page == 1)
                    adapter.datas.clear();
