...
public void initSearchJob(){
    ...
    try {
        List<SearchJobKeyCache> data = db.selector(SearchJobKeyCache.class).where("what", "=", WHAT).findAll();
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            ...
            case R.id.tv_search:
                ...
                try {
                    List<SearchJobKeyCache> temp = db.selector(SearchJobKeyCache.class).where("key_name", "=", key).findAll();
                    if (temp != null && temp.size() > 0)
                        db.delete(SearchJobKeyCache.class, WhereBuilder.b().and("key_name", "=", key));
                    SearchJobKeyCache searchJobKeyCache = new SearchJobKeyCache();
                ...
                public void onItemClick(Object o, int position) {
                    if (position != AlertView.CANCELPOSITION) {
                        try {
                            ...
                            db.dropTable(SearchJobKeyCache.class);
                        } catch (DbException e) {
                            e.printStackTrace();
                        }
                    }
